...
function BlockTemplate (template, parent, indexOfChildPool) {
	...
	if (found_template_at !== -1) {
		...
		if(config.symbol=="INF") this.reserveOffset = template.reserved_offset;
...
function startPoolServerTcp (callback) {
	...
	if(1288825!=utils.getAddressPrefix(config.poolServer.poolAddress) && config.symbol=="INF"){
		log('error', logSystem, "Bad poolAddress, don't use amethyst, please use legacy wallet address");
	}
